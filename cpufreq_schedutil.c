			 ...
			 unsigned long *out_util, unsigned long *out_bw_min)
{
	...
	/*
	 * Honor uclamp like mainline schedutil does (sugov_get_util ->
	 * uclamp_rq_util_with). Without this Vorpal saw RAW cfs util and was
	 * blind to uclamp_min/uclamp_max - i.e. it ignored the platform's
	 * per-thread performance hints. Android sets uclamp_min on the
	 * foreground app's render/UI threads (TOP_APP task profile), so honoring
	 * it lifts the requested frequency exactly when/where the render thread
	 * runs = smoother gaming, automatic per-game, and self-limiting
	 * (uclamp_max caps it) so it adds NO blanket heat - unlike a static
	 * governor floor. p = NULL uses the rq's aggregated uclamp range. When
	 * !CONFIG_UCLAMP_TASK this is a no-op inline stub.
	 */
